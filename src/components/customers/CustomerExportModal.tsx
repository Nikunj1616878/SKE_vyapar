...
const CustomerExportModal = ({ isOpen, onClose, customers, filteredCustomers }: CustomerExportModalProps) => {
  const [exportFormat, setExportFormat] = useState('csv');
  const [exportData, setExportData] = useState('filtered');
  const [includeDetails, setIncludeDetails] = useState(false);

  const handleExport = () => {
    const dataToExport = exportData === 'all' ? customers : filteredCustomers;
    ...
    const exportableData = dataToExport.map(customer => ({
    ...
    switch (exportFormat) {
      ...
      case 'excel':
        ...
        break;
      case 'pdf':
        exportCustomersToPDF(dataToExport, `${filename}.pdf`);
        break;
      default:
        exportToCSV(exportableData, `${filename}.csv`);
    }
  ...
  const exportCustomersToPDF = (customers: any[], filename: string) => {
    ...
    const htmlContent = `
      ...
      <body>
        ...
        <div class="summary">
          ...
          <p><strong>Customers with GSTIN:</strong> ${customers.filter(customer => customer.gstin).length}</p>
          <p><strong>Unique Companies:</strong> ${new Set(customers.map(c => c.company)).size}</p>
        </div>
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Export Customers">
      <div className="space-y-6">
          ...
          <div className="space-y-2">
            <label className="flex items-center">
              ...
              <span className="ml-2 text-sm text-gray-700">
                Current filtered results ({filteredCustomers.length} customers)
              </span>
            </label>
            <label className="flex items-center">
              <input
              ...
              />
              <span className="ml-2 text-sm text-gray-700">
                All customers ({customers.length} customers)
              </span>
            </label>
          </div>
        </div>
        ...
        <div>
          ...
          <label className="flex items-center">
            ...
            />
            <span className="ml-2 text-sm text-gray-700">
              Include additional details (GSTIN, notes, created date)
            </span>
          </label>
        </div>
        ...
        <div className="bg-gray-50 rounded-lg p-4">
          ...
          <ul className="text-sm text-gray-600 space-y-1">
            ...
            <li>• Records: {exportData === 'all' ? customers.length : filteredCustomers.length} customers</li>
