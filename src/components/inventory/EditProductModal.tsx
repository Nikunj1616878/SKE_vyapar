...
const EditProductModal = ({ isOpen, onClose, productId }: EditProductModalProps) => {
  const { inventory, updateProduct, getBrands } = useInventory();
  ...
  useEffect(() => {
    if (product && isOpen) {
      ...
      setShowCustomBrand(!existingBrands.includes(product.brand));
    }
  }, [product, isOpen, existingBrands]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const updatedProduct = {
      ...
      price: `₹${parseFloat(formData.price).toLocaleString('en-IN')}`,
      stock: parseInt(formData.stock),
      description: formData.description,
      sku: formData.sku,
      minStock: formData.minStock ? parseInt(formData.minStock) : undefined,
      status: getStockStatus(parseInt(formData.stock), formData.minStock ? parseInt(formData.minStock) : 5),
    };
